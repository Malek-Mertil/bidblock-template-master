...
import { EventTypeForm } from "@/types/locations/eventTypes";
import { z } from "zod";
...
const eventTypeSchema = z.object({
  name: z
    .string()
    .min(1, { message: "EventType name is required" })
    .max(50, { message: "EventType name must not exceed 50 characters" }),
});

export async function postEventType(state: EventTypeForm, formData: FormData): Promise<EventTypeForm> {
  // Extract form data
  const rawData = {
  ...
  try {
    ...
    const validatedData = eventTypeSchema.safeParse(rawData);
    ...
    if (!validatedData.success) {
     ...
     const response = await mutate<{ _id: string }>(
      `/event-types${rawData.id ? "/" + rawData.id : ""}`,
      rawData.id ? "PUT" : "POST",
      {
        name: rawData.name,
      }
    ...
    return generateState({}, undefined, "EventType saved successfully!");
  } catch (e) {
